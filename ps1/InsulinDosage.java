...
public class InsulinDosage {
    public static void main(String[] args) {
         ...
         */
        double sugar = (currentSugar - targetSugar) / 55; // computes the first part of the equation
        double carb = carbConsume / (double)carbEquiv; // computes the second part
        double dosage = sugar + carb - exercise; // puts together all of the parts of the equation minus the exercise variable

        if (dosage == 1.0){ // first if condition: if the dosage is 1.0
            System.out.println("recommended dosage: 1 unit");
        } else if (dosage > 0.0 && dosage % 1 == 0) { // second if condition: if the computed dosage is greater than 0.0 and is a whole number other than 1.0
            System.out.println("recommended dosage: " + (int) dosage + " " + "units"); 
        } else if (dosage > 0.0 && !(dosage % 1 == 0)) { // third if condition: if the computed dosage is greater than 0.0 and is not a whole number 
            System.out.println("recommended dosage: " + dosage + " " + "units");
        } else{ // last condition else: if the dosage is less than or equal to 0.0
