...
public class Sudoku {    
    ...
    // checks to see if either the row or column contains a value
    ...
    private boolean solveRB(int n) {
        ...
        // base case: if all the cells are filled up, then the puzzle is solved
        if (n == 81) {
            return true;
        } 
        
        if (valIsFixed[row][col]) { // checks for fixed cells, if it is fixed, then it moves to the next cell
            return solveRB(n+1);
        }
        ...
        for (int val = 1; val <= 9; val++) {
            if (!rowHasVal[row][val] && !colHasVal[col][val] && !subgridHasVal[row/3][col/3][val]) {
                ...
                if (solveRB(n+1)) { // recursion step
                    return true;
                }
                removeVal(val, row, col); // backtracking step
            }
        }
