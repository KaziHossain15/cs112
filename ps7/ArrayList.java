...
public class ArrayList implements List {
    ...
    public void rotate(int k) {
        ...
        // finds the new indexes and copies the newly arranged array into the temporary array
        for (int i = 0; i < length; i++) {
            int newIndexNum = i + k;
            int newIndex = newIndexNum % length; 
            tempArray[newIndex] = items[i]; 
        }

        // copies the temporary array to the old array in the new order
        for (int i = 0; i < length; i++) {
            items[i] = tempArray[i];
        }
    }
