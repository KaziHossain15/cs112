...
public class LLList implements List {
    ...
    /*
     * rotate method - rotates the array by the parameter integer k
     */
    public void rotate(int k) {
        if (k < 0 || k > length) { // check for illegal aruegment
            throw new IllegalArgumentException();
        }

        if (k == 0 || k == length) { // if k == 0 or length
            return;
        }

        Node currentNode = head.next; // keeps track of current node
        int numTrav = k-1; // number of traversals into the new arrangement
        for (int i = 0; i < k-numTrav; i++) { // iterate for every val
            currentNode = currentNode.next;
        }
        
        Node lastNode = currentNode; // assign last node to current node 
        while (currentNode.next != null) { // check if next node is not empty
            currentNode = currentNode.next; // assign if not empty
        }
        currentNode.next = head.next; // if it is empty, assign it to head.next since it is empty
        head.next = lastNode.next; // then head.next is last nodes next so it rotates
        lastNode.next = null; // then set last node's next value to null since it needs to be
    }
